...
| `runtime/help-article-screenshot-runner.mjs` | Playwright screenshot runner for preview-first help article screenshot capture. |
| `runtime/help-article-staging-auth-state.mjs` | Playwright staging login helper that writes runtime-only storage state from approved staging credentials. |
...
| `runtime/windmill/launchbot_ker_help_article_on_shipped.mjs` | Windmill entrypoint for Jira `KER-*` transitions into `6 - Shipped & Launching`, bilingual Intercom draft orchestration, Slack review, feedback, and exact-confirmation publish. |
| `runtime/windmill/launchbot_help_article_runs.sql` | Windmill state table schema for idempotent help article runs. |
| `skills/help-article-generator/` | Launchbot bilingual English/Indonesian help-article drafting skill upgraded from the 2026-05-11 handoff. |
| `skills/help-article-validator/` | Validates help article drafts against model StaffAny articles, evidence grounding, completeness, Intercom hygiene, and confidence scoring. |
| `skills/help-article-feedback-updater/` | Updates help article drafts from validator feedback, model-fit issues, evidence gaps, and Product Lead feedback before rerunning validation. |
| `skills/help-article-screenshot-capture/` | Launchbot screenshot planning and capture procedure for help article assets. |
| `skills/help-article-screenshot-troubleshooter/` | Screenshot capture diagnostics for Playwright, staging credentials, storage state, and blocker manifests. |
| `skills/product-marketing-launch-workflow/` | PMM launch workflow orchestrator for Jira KER Roadmap rows, priority mapping, and material status tracking. |
| `skills/launch-priority-identifier/` | Parses Jira-to-Slack LaunchBot commands, normalizes KER Launch Priority from `customfield_10561`, and routes release materials. |
| `skills/customer-support-release-notes-generator/` | Drafts concise CS-facing release notes for shipped KER tickets using Module / What's new / How this helps users / Setup / Help article link. |
| `skills/customer-support-release-notes-validator/` | Validates CS release notes with evidence-based reasoning, confidence scoring, pass/revise/blocked decisions, and required fixes. |
| `skills/customer-support-release-notes-feedback-updater/` | Updates CS release notes from validator feedback and reroutes them for a fresh validation pass. |
...
Install the Windmill state table before enabling the Jira webhook:

```bash
psql "$WINDMILL_DATABASE_URL" -f apps/launchbot/runtime/windmill/launchbot_help_article_runs.sql
```

Create Windmill flow `launchbot_ker_help_article_on_shipped` from `runtime/windmill/launchbot_ker_help_article_on_shipped.mjs`. The Jira Automation rule should trigger on work item transitions where project is `KER` and destination status is exactly `6 - Shipped & Launching`, then send immutable event data only:
...
```json
{
  ...
  "transition_to": "6 - Shipped & Launching",
  "transitioned_at": "2026-06-02T10:00:00.000+0800",
  "actor_account_id": "..."
}
```

The webhook entrypoint is `main(payload, context)`. The Slack continuation endpoints are exported as `apply_help_article_feedback(input)` and `publish_help_articles(input)`. The flow fetches fresh Jira fields itself, maps Product Lead to Slack, checks Pantheon/Intercom gates through the configured article pipeline, writes English and Indonesian Intercom drafts, posts review to `#launch-bot-testing`, accepts thread feedback only when `@Launch Bot` is mentioned, and publishes only after exact Product Lead confirmation: `@Launch Bot publish help articles KER-123`.
...
Screenshot capture is preview-first, demo-data only, and optional for the core article workflow. Create a shot list, dry-run it, then capture from an approved DEV/staging or local Gryphon source:

```bash
node apps/launchbot/runtime/help-article-screenshot-runner.mjs \
  --plan apps/launchbot/output/help-articles/screenshot-plans/create-and-manage-disbursement-scheduled-disbursement.json \
  --dry-run \
  --output-dir apps/launchbot/output/help-articles/assets/create-and-manage-disbursement
```

If capture is blocked by login, missing demo data, feature flags, browser availability, or redaction uncertainty, keep placeholders and record the blocker. Do not fail article drafting, Google Docs review, Slack approval, or Intercom draft/staging solely because screenshots are unavailable.

For staging troubleshooting with Playwright, hydrate approved staging credentials into the child process only, create runtime-only storage state, then pass that state to the screenshot runner:
...
The storage-state file contains session material. Keep it out of the repo and delete it after troubleshooting.
