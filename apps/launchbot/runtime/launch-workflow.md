...
- Required behavior:
  ...
  - Name the proven lane: code-grounded English and Indonesian help article drafts, Google Docs review drafts, Slack approval routing, Intercom draft articles after approval, and Customer Support release-note drafts with validator confidence scoring.
  - State that the PMM workflow is scoped to help articles and Customer Support release notes.
...
- Target outputs for Phase 1: help article work items and Customer Support release notes with evidence-based validation.
...
- Use `customer-support-release-notes-generator` for release notes.
- Every release note draft must pass through `customer-support-release-notes-validator`.
- If validation returns `revise`, use `customer-support-release-notes-feedback-updater`, then rerun the validator before review.
...
- PMM workflow launch derivatives are scoped to help article work items and Customer Support release notes with validator checkpoints only; changelog and WhatsApp Community are out of scope.
