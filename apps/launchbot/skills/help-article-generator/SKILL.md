...
1. Show draft text in-chat first, grouped by locale when bilingual output is in scope.
...
- Each locale record must include title, subtitle, publishable body, internal notes, evidence-check result, format-check result, Google Doc URL or review artifact path, Slack review message timestamp, Intercom draft/staging status, and final status.
