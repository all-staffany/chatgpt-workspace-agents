...
- Map Jira KER Roadmap rows into product-marketing launch work items for help articles and Customer Support release notes, with help-article and release-note validator confidence checkpoints.
...
- Use `customer-support-release-notes-generator` for concise CS-facing release notes after priority identification when release notes are needed.
- After release notes are drafted, run `customer-support-release-notes-validator` for evidence-based reasoning and a 0-100 confidence score.
- If validation returns `revise`, run `customer-support-release-notes-feedback-updater`, then validate again before review.
