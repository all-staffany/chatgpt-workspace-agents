---
name: customer-support-release-notes-generator
description: Generate concise StaffAny customer-support release notes from a shipped Jira KER ticket, Launch Priority, verified UI/UX behavior, and optional help article link. Use after launch-priority-identifier or when a Jira-to-Slack command asks LaunchBot for release notes for CS/support enablement.
---

# Customer Support Release Notes Generator

Generate short release notes for the Customer Support team from a shipped Jira KER ticket.
...
- Be concise. Write for CS agents who need to recognize, explain, and support the change quickly.
...
- Use plain customer-support language, not marketing copy.
...
- Keep each section to 1-2 bullets unless the ticket has multiple user-facing changes.
...
After drafting, immediately run `customer-support-release-notes-validator` with:
...
If the validator returns `revise`, run `customer-support-release-notes-feedback-updater`, then validate again. If the validator returns `blocked`, stop and name the missing evidence instead of revising from inference.
